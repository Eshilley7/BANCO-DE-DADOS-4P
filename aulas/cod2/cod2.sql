-- Active: 1787700300053@@127.0.0.1@5432@bd_aula@public
...
SELECT aluno_nome, turma, disciplina, nota FROM notas_alunos;

SELECT nota, disciplina, aluno_nome FROM notas_alunos ORDER BY nota DESC LIMIT 10;

SELECT disciplina, aluno_nome, turma, nota FROM notas_alunos WHERE nota >= 90;

SELECT disciplina, aluno_nome, turma, nota FROM notas_alunos WHERE nota <= 59 ORDER BY nota DESC;

SELECT disciplina, aluno_nome, nota FROM notas_alunos WHERE disciplina = 'Matematica' AND nota >= 70;

SELECT disciplina, aluno_nome, nota FROM notas_alunos WHERE disciplina = 'Portugues' AND nota >= 60;

SELECT disciplina, AVG(nota) AS "Média" FROM notas_alunos GROUP BY disciplina;

SELECT disciplina, COUNT (*) AS "Avaliações", AVG(nota) AS "Média" FROM notas_alunos GROUP BY disciplina;

SELECT disciplina, COUNT (*) AS "Avaliações", ROUND (AVG(nota), 2) AS "Média" FROM notas_alunos GROUP BY disciplina;