-- Active: 1788910601723@@127.0.0.1@5432@bd_vendas@public
...
SELECT 
    venda_id, quantidade, unidade, valor_unitario, data_venda 
FROM 
    vendas_itens2 
WHERE 
    produto_id = 1;
...
SELECT 
    venda_id, produto_id, quantidade, unidade, valor_unitario, data_venda
FROM 
    vendas_itens2 
WHERE 
    venda_id = 2001;
...
SELECT
    produto_id, SUM(quantidade) AS qtd_total, ROUND(SUM(quantidade * valor_unitario), 2) AS total_vendas,ROUND(SUM(quantidade * valor_unitario) / SUM(quantidade), 2) AS valor_medio, unidade
FROM
    vendas_itens2
GROUP BY
    produto_id, unidade
...
SELECT 
    venda_id, SUM(quantidade * valor_unitario) AS valor_total_venda
FROM
    vendas_itens2
GROUP BY
    venda_id, data_venda 
